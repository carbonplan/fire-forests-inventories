...
def return_gfed_emissions(
    start_year: int = 1997, end_year: int = 2022, return_spatial: bool = False
) -> pd.DataFrame:
    '''
    ...
    for year in tqdm(range(start_year, end_year + 1)):
            ...
            try:
                ...
                if partitioning:
                    for source in range(6):
                        ...
                        string = (
                            '/emissions/' + months[month] + '/partitioning/C_' + sources[source]
                        )
                        ...
                        C_emissions += C_emissions * contribution
                else:
        ...
        if gfeds_regions:
            CO2_table = pd.DataFrame(
                np.zeros((15, end_year - start_year + 1)),
                index=pd.date_range(start_year, end_year + 1, freq='A'),
            )  # region, year
